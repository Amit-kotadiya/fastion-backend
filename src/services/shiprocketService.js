...
function getCategoryDimensions(typeHint) {
  if (typeHint.includes("combo")) return { length: 20, breadth: 15 };
  if (typeHint.includes("pant") || typeHint.includes("trouser") || typeHint.includes("jean")) {
    return { length: 20, breadth: 15 };
  }
  if (typeHint.includes("tshirt") || typeHint.includes("t-shirt")) return { length: 14, breadth: 12 };
  if (typeHint.includes("shirt")) return { length: 14, breadth: 12 };
  if (typeHint.includes("underwear") || typeHint.includes("brief") || typeHint.includes("innerwear")) {
    return { length: 12, breadth: 10 };
  }
  if (typeHint.includes("shoe")) return { length: 30, breadth: 20 };
  if (typeHint.includes("watch")) return { length: 10, breadth: 10 };
  if (typeHint.includes("kurta") || typeHint.includes("hoodie")) return { length: 16, breadth: 14 };
  return { length: 14, breadth: 12 }; // default fallback
}
...
function estimatePackageDetails(items) {
  ...
  items.forEach((item) => {
    ...
    const dims = getCategoryDimensions(typeHint);
...
async function createShiprocketOrder(order) {
  ...
  await updateOrderShiprocketData(order.id || order.orderId, {
    shiprocketOrderId: srOrder.order_id != null ? String(srOrder.order_id) : null,
    shiprocketShipmentId: srOrder.shipment_id != null ? String(srOrder.shipment_id) : null,
